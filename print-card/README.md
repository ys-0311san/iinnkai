...
アクセア定形名刺向けに、55mm x 91mm 仕上がり、各辺3mm塗り足し込みの 61mm x 97mm PDF を生成します。既存サイトのファイルは参照のみで、生成物は `print-card/` 配下に閉じています。
...
- ページサイズは 61mm x 97mm。仕上がり 55mm x 91mm に対し、各辺3mmの塗り足しをページサイズで表現しています。
- アクセアの発注フォームでは “縦型名刺” のオプションを選んで入稿してください。
- トンボは付けていません。
- 断裁位置は各辺3mm内側です。
- 安全エリアは各辺6mm内側、中央 49mm x 85mm です。文字、ロゴ、QR はこの範囲内に収めています。
- 背景はページ端まで敷き切っています。
...
`images/card-logo.png` は 410 x 409px です。名刺全面に使うには不足するため、24mm 角の小さいヘッダーマークとして配置しています。

実効dpi計算:

```text
409px / (24mm / 25.4) = 約433dpi
```
